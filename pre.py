...
async def aiworkflow_subscriber(request: Request):
    """
    FastAPI endpoint to subscribe to the 'aiworkflow' Pub/Sub topic.
    This acts as a push subscription endpoint, receiving messages from GCP Pub/Sub.
    """
    envelope = await request.json()
    
    # Pub/Sub messages are wrapped in a 'message' field
    ...
    # and attributes are in the 'attributes' field
    ...
    if data:
        ...
        decoded_data = json.loads(base64.b64decode(data).decode("utf-8")) #-> list of jsons
        batch = []
    ...
    # For push subscriptions, Pub/Sub expects a 200 OK response to acknowledge receipt
    # Any other response code (or no response) will cause Pub/Sub to retry the message
    return {
        "status": "success",
